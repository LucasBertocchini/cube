...
function bruteForceSolve(order) {
    ...
    for (let subOrder = 1; subOrder <= order; subOrder++) {
        if (subOrder === 1) {
            ...
            for (let i = subOrder; i >= 0;) {
                ...
                const noRepeatedTurn = indices.reduce((prev, index) => {
            ...
            for (let i = subOrder; i >= 0;) {
                ...
                const noRepeatedIndex = indices.reduce((prev, index) => {
                    if (prev === false) return false;
                    else if (turns[prev].side === turns[index].side) return false;
                    return index;
                });
                ...
                for (i = subOrder; i--;) {
                    ...
                    if (noRepeatedIndex && i === subOrder - 1 && indices[subOrder - 2] < turnsLength - 1) {
                        let turn = turns[indices[subOrder - 2] + 1];
                        cubeList[subOrder - 2] = turnSide(cubeList[subOrder - 3], turn.side, turn.amount);
                    }
